local num_columns = GAMESTATE:GetCurrentStyle():ColumnsPerPlayer()
local width = (num_columns == 6 or num_columns == 12) and (224+1.75*6) or (num_columns == 8 or num_columns == 16) and (284+2.875*7) or (264+2*6)
...
local length = getenv("IIDXBeamLength"..pname(player)) or "normal"
...
return Def.ActorFrame{
	...
	Def.Sprite {
		...
		OnCommand=function(self) self:vertalign(reverse and top or bottom):y(reverse and 10 or -10) end
	},
	loadfile(NOTESKIN:GetPath("","_turntable"))(reverse)..{ InitCommand=function(self) self:y(reverse and -16 or 16) end },
