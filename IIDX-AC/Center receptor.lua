...
if beam == "random" then
	...
	local beams = { "none", "default", "orange", "pink", "monochrome", "onlyonebeam", "copula", "cannonballers", "heroicverse", "bistrover", "fresnelbeam", "resident", "epolis", "pinkycrush", "sparkleshower" }
...
return Def.ActorFrame{
	Def.Quad { OnCommand=function(self) self:zoomto(40,9999):vertalign(reverse and bottom or top):diffuse(color("#FFFFFF")):diffusealpha(background) end },
	Def.Quad { OnCommand=function(self) self:zoomto(2,9999):vertalign(reverse and bottom or top):x(20):diffuse(color("#3B3B3B")) end },
	Def.Quad { OnCommand=function(self) self:zoomto(2,9999):vertalign(reverse and bottom or top):x(-20):diffuse(color("#3B3B3B")) end },
	Def.Sprite {
		Texture="base light",
		Frame0000=0,
		Delay0000=1,
		InitCommand=function(self) self:zoom(not reverse and 1 or -1):addy(not reverse and 20 or -20):effectclock("beat"):zoomtowidth(40):blend('BlendMode_Add'):diffuseramp():effectcolor1(color("1,1,1,.2")):effectcolor2(color("1,1,1,1")):rotationz(180) end
	},
	Def.Sprite {
		Texture="RED_LINE",
		InitCommand=function(self) self:zoomtowidth(40) end
	},
	Def.Sprite {
	...
	Def.Sprite {
		...
		OnCommand=function(self) self:zoomtowidth(40):vertalign(not reverse and top or bottom):y(not reverse and -5 or 5):rotationz(180) end
	},
	Def.Sprite {
